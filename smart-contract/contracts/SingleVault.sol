...
contract SingleVault {
    ...
    // -------------------------------Events--------------------------------------------------//
    event GoalInitialized(address indexed creator, uint256 indexed goalId, string goalName, uint256 goalAmount, uint256 startTime, uint256 endTime);
    event SavingMade(address indexed saver, uint256 amount, uint256 totalSaved, uint256 timestamp);
    event GoalAchieved(address indexed creator, uint256 indexed goalId, uint256 totalAmount, uint256 timestamp);
    event WithdrawalMade(address indexed recipient, uint256 amount, uint256 platformFee, uint256 timestamp);
    event EmergencyWithdrawal(address indexed recipient, uint256 amount, uint256 emergencyFee, uint256 timestamp);
    ...
    function initialize(string memory _goalName, uint _goalAmount, uint _goalId, SavingsFrequency _savingFrequency, address currency, uint _startTime, uint _endTime, uint _platformFee, uint _emergencyFee, address _creator, address _circleVault, uint256 _participant) public {
        ...
        (, uint amountPerPeriod, ) = calculateSavingsSchedule(_goalAmount, _startTime, _endTime, _savingFrequency);

        IndividualGoal memory _goal = IndividualGoal(_creator, currency, _goalId, _goalName, _goalAmount, 0, amountPerPeriod, false, _startTime, _endTime, 0, _savingFrequency);
        individualGoals = _goal;
        
        emit GoalInitialized(_creator, _goalId, _goalName, _goalAmount, _startTime, _endTime);
    }
    ...
    function saveForGoal(address userAddr) public {
        ...
        IERC20(goal.currency).safeTransferFrom(msg.sender, address(this), goal.amountPerPeriod);
        ...
        emit SavingMade(userAddr, goal.amountPerPeriod, goal.amountSaved, block.timestamp);
        
        // Check if goal is achieved
        if (goal.amountSaved >= goal.goalAmount) {
            goal.goalAchieved = true;
            emit GoalAchieved(goal.goalCreator, goal.goalId, goal.amountSaved, block.timestamp);
        }
    }
    ...
    function payDefault(uint256 period) public {
        ...
        emit SavingMade(msg.sender, goal.amountPerPeriod, goal.amountSaved, block.timestamp);
    ...
    function _getFrequency(SavingsFrequency _frequency) private pure returns (uint256 freq) {
    ...
    function withdraw() public {
        ...
        emit WithdrawalMade(msg.sender, userAmount, _platformFee, block.timestamp);
    }
    ...
    function emergencyWithdrawal() public {
        ...
        emit EmergencyWithdrawal(msg.sender, userAmount, _emergencyFee, block.timestamp);
    }
     ...
     */
    function getIndividualGoal() external view returns (IndividualGoal memory goal) {
